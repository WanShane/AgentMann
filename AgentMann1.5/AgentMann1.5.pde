...
float[] moveRadians_horizontal = new float[65];
float[] moveRadians_vertical = new float[65];
...
void draw() {
  ...
  //I want to put those line on 3D space
  ...
  //left:Head Pose: pos(-0.05,-0.04,+0.58) rot(+9.71,+16.35,-1.99) conf +0.60

  //Right:Head Pose: pos(+0.02,-0.05,+0.62) rot(+3.64,-16.25,-0.53) conf +0.67
  ...
  /*
   ...
   //if you press the "k" key, the agent can move close to user.
   if (key == 'k') {
   if (i<=250) {
   translate(0, 0, i);
   }
